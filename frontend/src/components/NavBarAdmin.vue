...
<template>
  <nav class="sticky top-0 z-50 w-full py-2 shadow bg-white/75 backdrop-blur-md px-5">
    <div class="flex items-center justify-between max-w-6xl m-auto">
      <router-link
        class="flex min-w-40 h-10 gap-4 items-center hover:scale-110 transition"
        to="/admin"
      >
      ...
      <div class="gap-10 hidden sm:flex h-10 items-center">
        <router-link to="/admin/addremove">
          <Button v-if="isAdmin" variant="default">Administrer moderatorer</Button>
        </router-link>
        <ResetPasswordLink v-if="isAdmin" variant="default" />
      </div>
